...
const DashboardView = ({ stats, alerts, onNavigate }) => {
  ...
  const getSystemStatus = () => {
    const onlinePatients = stats?.active_patients || 0;
    const totalPatients = stats?.total_patients || 0;
    const criticalAlerts = stats?.critical_alerts_today || 0;
    
    if (criticalAlerts > 5) return { status: 'critical', message: 'Multiple critical alerts require attention' };
    if (criticalAlerts > 0) return { status: 'warning', message: 'Some patients require monitoring' };
    if (onlinePatients < totalPatients * 0.8) return { status: 'warning', message: 'Some patients are offline' };
        ...
        <div className="kpi-grid">
          <div className="kpi-card primary-card" onClick={() => handleStatCardClick('patients')}>
            ...
            <div className="kpi-content">
              <div className="kpi-number">{stats?.total_patients || 0}</div>
          ...
          <div className="kpi-card success-card" onClick={() => handleStatCardClick('patients')}>
            ...
            <div className="kpi-content">
              <div className="kpi-number">{stats?.active_patients || 0}</div>
          ...
          <div className="kpi-card warning-card" onClick={() => handleStatCardClick('alerts')}>
            ...
            <div className="kpi-content">
              <div className="kpi-number">{stats?.critical_alerts_today || 0}</div>
              <div className="kpi-label">Critical Alerts</div>
              <div className="kpi-sublabel">Require attention</div>
            </div>
            <div className="kpi-visual">
              <div className="alert-indicator ${stats?.critical_alerts_today > 0 ? 'active' : ''}">
          ...
          <div className="kpi-card info-card" onClick={() => handleStatCardClick('alerts')}>
            ...
            <div className="kpi-content">
              <div className="kpi-number">{stats?.unresolved_alerts || 0}</div>
            ...
            <div className="vitals-display">
              <div className="vital-metric heart-rate">
                ...
                <div className="metric-info">
                  <div className="metric-value">{stats?.avg_heart_rate_today || 72}</div>
              ...
              <div className="vital-metric oxygen-level">
                ...
                <div className="metric-info">
                  <div className="metric-value">{stats?.avg_oxygen_level_today || 98}</div>
                  <div className="metric-label">Avg Oxygen</div>
                  <div className="metric-unit">%</div>
                </div>
                <div className="metric-gauge">
                  <div className="gauge-fill" style={{ width: `${stats?.avg_oxygen_level_today || 98}%` }}></div>
                </div>
              </div>
            </div>
          </div>
